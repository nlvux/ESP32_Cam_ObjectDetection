...
String classify() {
  ...
  for (size_t ix = 0; ix < EI_CLASSIFIER_LABEL_COUNT; ix++) {
    ...
    if (result.classification[0].value > 0.8) // Bật LED đỏ nếu thấy vật thể lạ
      { digitalWrite(RED, HIGH); ledOn = true; } 
    else { digitalWrite(RED, LOW); ledOn = false; }
    
  if (result.classification[1].value > 0.8) // Bật LED xanh nếu thấy vệ tinh
      { digitalWrite(GREEN, HIGH); ledOn = true; } 
    else { digitalWrite(GREEN, LOW); ledOn = false; }
